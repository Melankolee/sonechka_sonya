...
Три штуки, все только на этой странице:

**Из бокала можно пить.** Тап по пене или по пиву под ней — глоток: шапка вместе
с поверхностью уходит вниз, а на освободившемся стекле остаётся кольцо там, где
пена стояла. Пять глотков — и уровень дальше не идёт, следующий тап наливает
заново. Тап ловится на `document`, а не на холсте (тот лежит `pointer-events:
none`, чтобы не отбирать прокрутку), и засчитывается, только если палец не уехал
дальше 10px — иначе это было листание. Всё в [site/beer-glass.js](site/beer-glass.js):
`sip`, `refill`, `emptyGlass` (пустая часть бокала) и `lacing` (кольца).

**Счёт бокалов.** Выпитое копится в `localStorage` (`sonechka:beers-kostya`) и
переживает закрытие страницы. Считает вслух строчка под заголовком — та самая,
где до первого глотка написано «наклони телефон».

**Блокнот на три листа.** Оторвал все — остаётся картонная обложка с наклейкой,
и мысли больше не отправить, кнопка говорит «Листы кончились». Счёт живёт в
state, а не в хранилище: перезагрузил — блокнот новый. Иначе доигравшийся Костя
навсегда лишился бы возможности написать по делу.

Плюс мелочь: у карточки «Я приду» заломлен верхний угол — если отклеить, под ним
надпись. Складка — поворот треугольника вокруг оси собственной диагонали
(`peelStyle` в [site/kostya.html](site/kostya.html)).
